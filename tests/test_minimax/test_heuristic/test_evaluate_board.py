...
def test_evaluate_board():
    ...
    1- this board is an example that shows the benefit of evaluating each axis separately.
    such that we can check that for example X can reach a tow-way-win by placing a piece on (3,4).
    ...
    avoids this board becasue O has two connect three on the board. this should be improved by checking
    the immediate pivots first and weight them differently. (not done yet due to time limit)
    summary: the agent should weight the X position better than O. but it doesn't due to design of
